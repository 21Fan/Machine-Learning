...
##conv1 layer##
# patch 5x5,in size 1是image的厚度,out size 25高度
W_conv1 = weight_variable([5,5,1,25])
b_conv1 = bias_variable([25])
#非线性处理(x*W+b)
h_conv1 = tf.nn.relu(conv2d(x_image,W_conv1)+b_conv1)#out size=28x28x25
h_pool1 = max_pool_2x2(h_conv1)                     #out size=14*14*25

##conv2 layer##
# [patch 5x5,in size 25是输入厚度,out size 50高度]
W_conv2 = weight_variable([5,5,25,50])
b_conv2 = bias_variable([50])
h_conv2 = tf.nn.relu(conv2d(h_pool1,W_conv2)+b_conv2)#out size=14*14*50
h_pool2 = max_pool_2x2(h_conv2)                     #out size=7*7*50

##func1 layer##
W_fc1 = weight_variable([7*7*50,150])
